        ...
        .then(results => {
            ...
            for (const result in results) {
                for (const sm in results[result]) {
                    ...
                    // kurrencyName.push(results[result][sm].currencyName, results[result][sm].currencyId);
                    //console.log(kurrencyName);
                    //  console.log(kurrencyName.sort());
...
function convertCurrency() {
    ...
    function createDB() {
        ...
        return idb.open(dbases, 1, upgradeDb => {
            ...
            // objectStore name is rateSheet  previously it was Rates
            console.log(' Now creating a new object store');

            if (!upgradeDb.objectStoreNames.contains('rateSheet', { keyPath: 'xId' })) {
                upgradeDb.createObjectStore(['rateSheet'])
                    .createIndex('refRateTxn', 'refRateTxn', { unique: true });
    ...
    function rateStorage() {
        ...
        return fetch(convUrl)
            ...
            .then(rata => {
                ...
                createDB().then(db => {
                        if (!db) return;
                        let txnx = db.transaction('rateSheet', 'readwrite');
                        let rateStore = txnx.objectStore('rateSheet');
                        ...
                        rateStore.add({
                            ...
                            //'xId': `${xID}`,
                            //'rateSheet': `${Object.keys(rata), Object.values(rata)}`
                            //'rateSheet': `${Object.values(rata)}`
                            //'rateSheet': `${JSON.stringify(rata)}`
                            'rateSheet': `${Object.entries(rata)}`
                        });
