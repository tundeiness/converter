...
/* function openDb() {
    ...
    const dbName = 'countryRecords';
    const dbPromise = idb.open(dbName, 1, upgradeDb => {

        if (upgradeDb.oldVersion) {
            console.log("database is an old version of "
                upgradeDb.oldVersion);
        } else {
            upgradeDb.creatObjectStore('countries', { keyPath: 'countryId' });
        }
...
var dbProm = idb.open(dbName, 1, upgradeDb => {
    ...
    switch (upgradeDb.oldVersion) {
        ...
        case 1:
            ...
            upgradeDb.createObjectStore('countryRecords', { keyPath: 'countryId' });
...
fetch(apiURL)
    ...
    .then(data => {
        dbProm.then(db => {
                ...
                // let countriesCurrencies = [data.results];
                ...
                for (let currency in data) {
                    ...
                    for (let res in data[currency]) {
                        console.log(res);
                        countryStore.put(res["currencyId"]);
                        console.log(res["currencyId"]);
                    }
                }

                /* countriesCurrencies.forEach(currency => {
                    for (let value in currency) {
                        countryStore.put(value.currencyId);
                    }
                }) */
                return txn.complete;
            })
