 ...
 */
            //let ner = Object.values(objectArray.results).sort(currencyId);

            //console.log(objectArray);
            /*  for (const nu of objectArray) {
                 console.log(nu);
             } */

            // for (const currency of Object.values(objectArray.results).sort(
            //  CurrencyId
            // )) 
...
toggler.addEventListener('click', () => {
    ...
    //let totalAmt = `${to} ${(rfrmTo * inputAmount.value).toFixed(2)}`;
    ...
    fetch(ratesURL)
        ...
        .then(rata => {
            ratePromise.then(db => {
                    ...
                    rateStore.put({
                        'rateValue': `${Object.values(rata)}`,
                        'Rates': `${Object.keys(rata)}`
                    });
    ...
    /* 
        fetch(convUrl)
            ...
            .then(rata => {
                ...
                const rfrmTo = rata[`${from}_${to}`];
                const totalAmt = `${to} ${(rfrmTo)}`;
                // const totalAmt = `${to} ${(rfrmTo * inputAmount.value).toFixed(2)}`;
            ...
            .catch(err => {
                console.log("Rates is not storing", err);
            }); */
...
/* rateStore.put({
                    'rateValue': `${Object.values(convertCurrency())}`,
                    'Rates': `${Object.keys(convertCurrency())}`
                }); */
